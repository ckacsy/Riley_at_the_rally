...
| `admin_investigation.spec.ts` | E2E tests for admin investigation timeline and entity endpoints — access control (401 / 403 / 200; moderator+ for entity, admin-only for timeline), validation (no filters → 400; invalid user_id / car_id / date_from / page / limit → 400), timeline data from 4 sources (transaction, session, audit, maintenance) with correct shape and summary/details fields, filtering by user_id / car_id / reference_id / date range / combined filters, sorting DESC by created_at, pagination (page/limit/total/pages), and entity quick cards for user (id/username/email/status/role/balance), session (entity + related transactions via session_ref), and car (maintenance record or null + recent_sessions_count); entity error cases (unknown type → 400; invalid/nonexistent id → 400/404) |
